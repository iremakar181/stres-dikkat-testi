...
// Ses dosyası yolları, GitHub yükleme ekranınızdaki isimlerle (birebir büyük/küçük harf ve boşluklar dahil) eşleştirildi!
const lofiTracks = {
    gingersweet: new Audio('./audio/massobeats - gingersweet (freetouse.com).mp3'),
    time: new Audio('./audio/Avanti - Time (freetouse.com).mp3'),
    garden: new Audio('./audio/Aventure - Güzel Bir Bahçe (freetouse.com).mp3')
};
...
function toggleLofi(element) {
    ...
    if (parentCard.classList.contains('active')) {
        ...
        track.play().catch(e => {
            parentCard.classList.remove('active');
            console.error("Lo-Fi audio play error. Check case-sensitivity or Service Worker restrictions:", e);
        });
    }
}
...
function changeLofiTrack(selectElement) {
    ...
    if (lofiTracks[currentLofiTrackKey]) {
    ...
    if (wasPlaying && lofiTracks[currentLofiTrackKey]) {
        lofiTracks[currentLofiTrackKey].play().catch(e => {
            parentCard.classList.remove('active');
            console.error("Error changing and playing Lo-Fi track:", e);
        });
    }
}
...
function toggleTrack(trackName, element) {
    ...
    if (element.classList.contains('active')) {
        ...
        track.play().catch(e => {
            element.classList.remove('active'); 
            console.error(trackName + " ambience file not found or blocked. Check path casing:", e);
        });
    }
}
...
function changeLanguage(lang) {
    ...
    const dynamicKeys = ['score-ax-res', 'score-mem-res', 'score-stroop-res', 'score-text-res', 'phase-read', 'phase-fill'];

    document.querySelectorAll('[data-i18n]').forEach(element => {
        const key = element.getAttribute('data-i18n');
        if (dynamicKeys.includes(key)) return;

        if (translations[lang][key]) {
            if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
                element.setAttribute('placeholder', translations[lang][key]);
            } else {
                // Menülerdeki ikonları koruyan akıllı metin güncelleme
                if (element.children.length > 0) {
                    ...
                    if (textNode) {
                        const leadingSpace = textNode.nodeValue.startsWith(' ') || textNode.nodeValue.startsWith('\xa0') ? ' ' : '';
                        const trailingSpace = textNode.nodeValue.endsWith(' ') || textNode.nodeValue.endsWith('\xa0') ? ' ' : '';
                        textNode.nodeValue = leadingSpace + translations[lang][key] + trailingSpace;
                    } else {
                        let rawTextNode = Array.from(element.childNodes).find(n => n.nodeType === Node.TEXT_NODE);
                        if (rawTextNode) {
                            rawTextNode.nodeValue = translations[lang][key];
                        } else {
    ...
    if (currentQuoteIndex >= langQuotes.length) {
        currentQuoteIndex = 0;
    }
    displayQuote();
    ...
    try {
        ...
        console.error("Error parsing user profile on language change:", e);
    }
    ...
    try {
        ...
        console.error("Error updating dashboard on language change:", e);
    }
}
...
function popBubbleCluster(clickedBubble) {
    ...
    if (activeBubbles.length === 0) {
        setTimeout(() => { 
            const successMsg = currentLang === 'tr' ? "Harika Deşarj! Yeni Tahta Yükleniyor." : (currentLang === 'de' ? "Großartige Entlastung! Neues Brett lädt." : "Great Discharge! Loading new board.");
            alert(successMsg); 
            initBubblePopGame(); 
        }, 400);
    }
...
let testInt, letterInt, memoryTimeout;
let timeLeft = 60;
let hits = 0, wrongs = 0;
let targetLetter = 'A';
let currentLetter = '';
...
function startMemoryTest() {
    ...
    initBubblePopGame();
    clearTimeout(memoryTimeout);
    memoryTimeout = setTimeout(() => { endGlobalTest('memory'); }, 15000);
}
...
function nextStroopQuestion() {
    const wordEl = document.getElementById('stroop-word-box');
    if (!wordEl) return;
...
const textualDbPool = [
    {
        ...
        en: { text: "Umay Ana is the symbol of a universal aura in Turkish mythology that heals and protects minds from evil. The flutter of a butterfly's wings confirms how small decisions impact macro transformations in the future." },
        de: { text: "Umay Ana ist in der türkischen Mythologie das Symbol einer universellen Aura, die heilt und den Geist vor dem Bösen schützt. Der Flügelschlag eines Schmetterlings bestätigt, wie kleine Entscheidungen makroökonomische Veränderungen in der Zukunft beeinflussen." }
    }
];
...
function startTextFocusTest() {
    ...
    const passage = textualDbPool[0][currentLang]?.text || textualDbPool[0]['tr'].text;
    document.getElementById('text-paragraph-display').innerText = passage;
...
function endGlobalTest(type) {
    ...
    if (type === 'ax-cpt') {
        ...
        resultText = translations[currentLang]["score-mem-res"].replace("{moves}", bubbleScore);
    } else {
...
function updateProfileUI(user) {
    ...
    if (welcome) welcome.innerText = `${translations[currentLang]["welcome-title"]} ${name} 👋`;
    if(document.getElementById("profile-display-name")) document.getElementById("profile-display-name").innerText = name;
    ...
    if(document.getElementById("avatar-letter")) document.getElementById("avatar-letter").innerText = name ? name.charAt(0).toUpperCase() : "?";
...
function resetAllData() { 
    const confirmMsg = currentLang === 'tr' ? "Sistemin tüm yerel state hafızası sıfırlansın mı?" : (currentLang === 'de' ? "Möchten Sie alle Systemdaten zurücksetzen?" : "Reset all local storage data?");
...
function saveDailyStatus() {
    ...
    let weekly = [];
    try {
        weekly = JSON.parse(localStorage.getItem('umiora_weekly')) || [];
    } catch(e) {
        console.error("Error loading weekly log:", e);
    }
...
function updateDashboard(weekly) {
    ...
    let stressLabel = currentLang === 'tr' ? 'Stres' : (currentLang === 'de' ? 'Stress' : 'Stress');
    let sleepLabel = currentLang === 'tr' ? 'Uyku' : (currentLang === 'de' ? 'Schlaf' : 'Sleep');
    let hourUnit = currentLang === 'tr' ? 'sa' : 'h';
    
    if(document.getElementById('home-status-text')) {
        document.getElementById('home-status-text').innerHTML = `${stressLabel}: <strong>${desc}</strong><br>${sleepLabel}: <strong>${last.hours}${hourUnit}</strong>`;
    }
    if(document.getElementById('home-focus-text')) document.getElementById('home-focus-text').innerText = `%${Math.round(last.score)}`;
    
    let dayText = currentLang === 'tr' ? 'Gün' : (currentLang === 'de' ? 'Tage' : 'Days');
    if(document.getElementById('profile-streak')) document.getElementById('profile-streak').innerText = `${weekly.length} ${dayText}`;
    
    let logText = currentLang === 'tr' ? 'Not' : (currentLang === 'de' ? 'Einträge' : 'Logs');
    let journalCount = 0;
    try {
        journalCount = (JSON.parse(localStorage.getItem('umiora_journal')) || []).length;
    } catch(e) {}
    if(document.getElementById('profile-journal-count')) {
        document.getElementById('profile-journal-count').innerText = `${journalCount} ${logText}`;
    }
}
...
function saveJournal() {
    ...
    let data = [];
    try {
        data = JSON.parse(localStorage.getItem('umiora_journal')) || [];
    } catch(e) {
        console.error("Error parsing journal logs:", e);
    }
...
function loadJournals() {
    ...
    try {
        ...
        console.error("Error loading journal logs:", e);
    }
    hist.innerHTML = data.map(e => `<div class="journal-entry"><div>${e.date}</div><div>${e.content}</div></div>`).join('');
}
...
function displayQuote() {
    ...
    const langQuotes = quotes[currentLang] || quotes['tr'];
    if (quoteElement) {
        const quote = langQuotes[currentQuoteIndex] || langQuotes[0] || "";
...
window.onload = function () {
    try {
        ...
        console.error("Umiora initialization error:", e);
    }
};
